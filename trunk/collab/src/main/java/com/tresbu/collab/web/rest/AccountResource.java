...
import com.tresbu.collab.web.rest.vm.KeyAndPasswordVM;
...
public class AccountResource {
    ...
    public ResponseEntity<?> getAccount() {
    	...
    	// Tenant object is required in the response of this api, as it is used in navbar.controller.js to hide change password link for trakeye users
        return Optional.ofNullable(userService.getUserWithAuthorities())
            .map(user -> new ResponseEntity<>(new UserUIDTO(user), HttpStatus.OK))
            .orElse(new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR));
        ...
        return userService.requestPasswordReset(mail)
            .map(user -> {
             ...
             // Mail should be sent only if user is not created by trakeye.
                if(!user.getTenant().getTrakeyeTenant()){
